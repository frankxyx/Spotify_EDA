---
title: "Exploratory Data Analysis of Spotify Streaming Trends"
author: "Frank Xiang"
date: "`r Sys.Date()`"
output: 
  pdf_document:
...
```{r}
spotify_data <- read.csv("/Users/frankmayfield/Documents/R_data/Most Streamed Spotify Songs 2024.csv")
# head(spotify_data)
...
top_20_streams <- sum(artist_streams$total_streams[1:20], na.rm = TRUE)
rest_streams <- sum(artist_streams$total_streams[-(1:20)], na.rm = TRUE)

# Create a new dataset for the pie chart
pie_data <- data.frame(
  Category = c("Top 20 Artists", "All Other Artists"),
  Total_Streams = c(top_20_streams, rest_streams)
)
...
The top four artists, The Weeknd, Bad Bunny, Drake, and Taylor Swift, have significantly higher total stream counts compared to the rest of the ranking. The Weeknd leads with over 37 billion streams, while artists like Dua Lipa and Maroon 5, ranked near the bottom of top 20, have around 16 billion streams which is less than half of the leader’s total. This massive disparity suggests a highly skewed distribution, likely following a power-law trend commonly seen in digital media, where a small number of artists dominate while the majority accumulate significantly fewer streams.

One interesting observation is the sharp drop in total streams after the top four artists. While artists like Post Malone, Ed Sheeran, and Ariana Grande still have impressive numbers, they fall far behind the leaders. This could indicate that the top four have achieved sustained global appeal across multiple albums and songs, rather than relying on a few viral hits.

Another notable trend is genre and market influence. Bad Bunny is the only Latin artist in the top 10, demonstrating the rising influence of Latin music worldwide. However, the continued dominance of English-language artists suggests that the Western music industry still holds the most influence in global streaming trends. The presence of artists like Eminem and Imagine Dragons, who peaked in earlier decades, also shows the long-lasting popularity of certain artists, likely due to catalog streaming—where older songs continue to accumulate streams over time.

A key question raised by this analysis is: do these artists maintain their ranking due to a consistent catalog of hits, or are they driven by a few massive viral songs? Artists like Drake and Taylor Swift have built their careers through multiple high-streaming albums, while emerging artists like Olivia Rodrigo may have reached the top due to a few blockbuster singles. A deeper look into Total Streams vs. Most Streamed Songs on each artist will provide more insights.
...
Another interesting takeaway is the presence of certain international artists, such as Bad Bunny, whose dominance highlights the increasing globalization of music. While English-speaking artists still lead the rankings, the fact that Latin music is achieving comparable streaming numbers suggests changing consumer behavior and the growing influence of non-English songs in global markets.
